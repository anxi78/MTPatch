package top.nkbe.mtpatch.service;
...
public class IntegrApplicationService extends ILSPApplicationService.Stub {
    ...
    public IntegrApplicationService(Context context) {
        try {
            String[] assetsList = context.getAssets().list("mtpatch/modules");
            ...
            for (var name : assetsList) {
                ...
                String modulePath = context.getCacheDir() + "/mtpatch/" + packageName + "/";
                String cacheApkPath;
                ...
                if (!Files.exists(Paths.get(cacheApkPath))) {
                    ...
                    try (var is = context.getAssets().open("mtpatch/modules/" + name)) {
