...
namespace lspd {
    ...
    static int CreateDexMemFd(const void* data, size_t size) {
        ...
        const int fd = syscall(__NR_memfd_create, "mtpatch_dex", MFD_CLOEXEC);
        if (fd < 0) {
            return -1;
        }
    ...
    void PatchLoader::SetupEntryClass(JNIEnv* env) {
        ScopedLocalRef<jclass> entry_class(FindClassFromLoader(env, GetCurrentClassLoader(), "top.nkbe.mtpatch.loader.LSPApplication"));
        if (entry_class) {
    ...
    void PatchLoader::Load(JNIEnv* env) {
        ...
        auto stub = JNI_FindClass(env, "top/nkbe/mtpatch/metaloader/LSPAppComponentFactoryStub");
