package top.nkbe.mtpatch.patch;

import static top.nkbe.mtpatch.share.Constants.CONFIG_ASSET_PATH;
import static top.nkbe.mtpatch.share.Constants.EMBEDDED_MODULES_ASSET_PATH;
import static top.nkbe.mtpatch.share.Constants.LOADER_DEX_ASSET_PATH;
import static top.nkbe.mtpatch.share.Constants.ORIGINAL_APK_ASSET_PATH;
import static top.nkbe.mtpatch.share.Constants.PROXY_APP_COMPONENT_FACTORY;
...
import top.nkbe.mtpatch.share.LSPConfig;
import top.nkbe.mtpatch.share.PatchConfig;
import top.nkbe.mtpatch.patch.util.ApkSignatureHelper;
import top.nkbe.mtpatch.patch.util.JavaLogger;
import top.nkbe.mtpatch.patch.util.Logger;
import top.nkbe.mtpatch.patch.util.ManifestParser;
...
public class MTPatch {

    private static final String MTPATCH_KEYSTORE_PASSWORD_ENC = "a2hpbm9s";
    private static final String MTPATCH_KEY_ALIAS_ENC = "MT8jag==";
    ...
    @Parameter(names = {"-npa", "--mtpatch-keystore"}, description = "Use built-in MTPatch keystore")
    private boolean useMtpatchKeystore = false;
    ...
    public MTPatch(Logger logger, String... args) {
        ...
        if (keystoreArgs != null && (useMtpatchKeystore || useFpaKeystore)) {
            logger.e("Cannot use -k with -npa or -fpa\n");
            help = true;
        }
        if (useMtpatchKeystore && useFpaKeystore) {
    ...
    public static void main(String... args) throws IOException {
        MTPatch mtpatch = new MTPatch(new JavaLogger(), args);
        if (mtpatch.help) {
            mtpatch.jCommander.usage();
            return;
        }
        try {
            mtpatch.doCommandLine();
        } catch (PatchError e) {
    ...
    public void doCommandLine() throws PatchError, IOException {
        for (var apk : apkPaths) {
            ...
            File outputFile = new File(outputDir, String.format(
                    Locale.getDefault(), "%s-%d-mtpatched.apk",
    ...
    public void patch(File srcApkFile, File outputFile) throws PatchError, IOException {
        ...
        try (ZFile dstZFile = ZFile.openReadWrite(outputFile, Z_FILE_OPTIONS);
            ...
            try {
                ...
                if (useMtpatchKeystore || (!useFpaKeystore && keystoreArgs == null)) {
                    logger.i("Register apk signer with built-in MTPatch keystore...");
                    registerBuiltinSigner(keyStore, dstZFile, "assets/mtpatch.key", MTPATCH_KEYSTORE_PASSWORD_ENC, MTPATCH_KEY_ALIAS_ENC);
            ...
            if (isInjectProvider){
                try (var is = getClass().getClassLoader().getResourceAsStream("assets/mtprovider.dex")) {
                    dstZFile.add("assets/mtpatch/mtprovider.dex", is);
                } catch (Throwable e) {
                    throw new PatchError("Error when adding dex", e);
                }
            }
            ...
            if (!useManager) {
                ...
                // do not put libmtpatch.so into apk!lib because x86 native bridge causes crash
                for (String arch : ARCHES) {
                    String entryName = "assets/mtpatch/so/" + arch + "/libmtpatch.so";
    ...
    private byte[] modifyManifestFile(InputStream is, String metadata, int minSdkVersion, String originPackage, String newPackage, String originalSignature) throws IOException {
        ...
        modules.forEach(module -> {
            ...
            property.addMetaData(new ModificationProperty.MetaData("xposeddescription", "MTPatch Embed Module"));
            property.addMetaData(new ModificationProperty.MetaData("xposedminversion", "93"));
        });

        property.addMetaData(new ModificationProperty.MetaData("mtpatch", metadata));
